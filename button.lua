...
function button:update()
	...
	if x < self.x + 10 + 20-1 and x > self.x and y < self.y + 10 + 20-1 and y > self.y then 
		if love.mouse.isDown(1) and self.text ~='M' then 
			self.code(self.x,self.y)
		end
    if love.mouse.isDown(2) then
      if self.colorChange and self.text ~='M' then 
      self.text = 'M'
      self.originalColor = {1,1,0}
      self.color = {1,1,0}
     elseif self.text =='M' then
       self.text = ''
       self.color = {1,0,0}
       self.originalColor = {1,0,0}
       end
      end
